...
describe('PracticeRunner round completion', () => {
  ...
  it.each(['/api/practice/attempts', '/api/practice/word-attempts'])('retains the exact result and request key when retrying %s', async (path) => {
    mockedApi.mockRejectedValueOnce(new Error('连接中断')).mockResolvedValueOnce({ id: 1, cpm: null, accuracy: 100, errors: 0, duration_ms: 100, speed_char_count: 0, metric_version: 2 })
    render(<MemoryRouter><PracticeRunner contextLabel="测试" title="重试" backLabel="返回" items={[{ id: 1, content: 'a' }]} savePath={path} saveIdKey={path.includes('word') ? 'word_id' : 'prompt_id'} /></MemoryRouter>)
    fireEvent.keyDown(window, { key: 'a', code: 'KeyA' })
    await act(async () => {})
    expect(screen.getByRole('alert')).toHaveTextContent('本次成绩仍保留')
    expect(screen.getByRole('button', { name: '重练' })).toBeDisabled()
    const unload = new Event('beforeunload', { cancelable: true })
    window.dispatchEvent(unload)
    expect(unload.defaultPrevented).toBe(true)
    await act(async () => { await vi.advanceTimersByTimeAsync(30000) })
    expect(mockedApi).toHaveBeenCalledTimes(1)
    fireEvent.click(screen.getByRole('button', { name: '重试保存成绩' }))
    await act(async () => {})
    expect(mockedApi.mock.calls[1][1]?.body).toBe(mockedApi.mock.calls[0][1]?.body)
    expect(JSON.parse(String(mockedApi.mock.calls[0][1]?.body)).request_id).toMatch(/^[a-f0-9]{32}$/)
    expect(screen.getByText('本轮完成！')).toBeInTheDocument()
    expect(screen.queryByRole('alert')).not.toBeInTheDocument()
  })

  it('blocks restart during a slow save and offers a retry after timeout', async () => {
    mockedApi.mockImplementation((_path, init) => new Promise((_resolve, reject) => {
      init?.signal?.addEventListener('abort', () => reject(new DOMException('Aborted', 'AbortError')))
    }))
    render(<MemoryRouter><PracticeRunner contextLabel="测试" title="超时" backLabel="返回" items={[{ id: 1, content: 'a' }]} savePath="/api/practice/attempts" saveIdKey="prompt_id" /></MemoryRouter>)
    fireEvent.keyDown(window, { key: 'a', code: 'KeyA' })
    expect(screen.getByRole('button', { name: '重练' })).toBeDisabled()
    fireEvent.click(screen.getByRole('button', { name: '重练' }))
    expect(screen.getByText('正在保存成绩…')).toBeInTheDocument()
    await act(async () => { await vi.advanceTimersByTimeAsync(15000) })
    expect(screen.getByRole('alert')).toHaveTextContent('保存超时')
    expect(screen.getByRole('button', { name: '重试保存成绩' })).toBeEnabled()
    expect(mockedApi).toHaveBeenCalledTimes(1)
  })

  it('does not schedule another round after a save finishes on an unmounted page', async () => {
    let complete: (value: unknown) => void = () => {}
    mockedApi.mockImplementation(() => new Promise((resolve) => { complete = resolve }))
    const view = render(<MemoryRouter><PracticeRunner contextLabel="测试" title="离开" backLabel="返回" items={[{ id: 1, content: 'a' }]} savePath="/api/practice/attempts" saveIdKey="prompt_id" /></MemoryRouter>)
    fireEvent.keyDown(window, { key: 'a', code: 'KeyA' })
    view.unmount()
    await act(async () => { await vi.advanceTimersByTimeAsync(1) })
    await act(async () => complete({ id: 1, cpm: null, accuracy: 100, errors: 0, duration_ms: 100, speed_char_count: 0, metric_version: 2 }))
