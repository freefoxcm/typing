...
function MarkdownEditorField({ label, value, onChange, readOnly, rows = 3, className = '', required = false }: { label: string; value?: string; onChange?: (value: string) => void; readOnly: boolean; rows?: number; className?: string; required?: boolean }) {
  if (readOnly) return <section className={`markdown-editor-field markdown-editor-readonly${className ? ` ${className}` : ''}`} aria-label={label}><h4>{label}</h4><MarkdownPreview value={value} readOnly /></section>
  return <div className={`markdown-editor-field${className ? ` ${className}` : ''}`}><label>{label}<textarea rows={rows} value={value ?? ''} onChange={(event) => onChange?.(event.target.value)} required={required} /></label><MarkdownPreview value={value} /></div>
}
...
function QuestionEditor({ value, setTitle, readOnly, currentPosition, queueSize, canPrevious, canNext, onCancel, onSave, onSaveReviewNext, onNavigate, onUploadImage, onUploadStemImage, onRemoveStemImage, onRecognize, recognitionAvailable, recognitionPending, recognitionProgress, recognitionStatus }: { value: EditableQuestion; setTitle: string; readOnly: boolean; currentPosition: number; queueSize: number; canPrevious: boolean; canNext: boolean; onCancel: () => void; onSave: (value: EditableQuestion) => Promise<void>; onSaveReviewNext: (value: EditableQuestion) => Promise<void>; onNavigate: (offset: number) => void; onUploadImage: (questionId: number, file: File) => Promise<ExerciseQuestion>; onUploadStemImage: (questionId: number, file: File) => Promise<ExerciseQuestion>; onRemoveStemImage: (questionId: number) => Promise<ExerciseQuestion>; onRecognize: (questionId: number) => Promise<void>; recognitionAvailable: boolean; recognitionPending: boolean; recognitionProgress?: JobProgress; recognitionStatus?: JobStatus }) {
  ...
  return <div className="modal-backdrop question-editor-backdrop" role="presentation"><form className={`question-editor-modal card${readOnly ? ' readonly' : ''}`} aria-label={readOnly ? '题目查看器' : '题目编辑器'} aria-readonly={readOnly} onSubmit={(e) => { e.preventDefault(); void save(false) }} onChangeCapture={(event) => { if (readOnly) { event.preventDefault(); event.stopPropagation() } }} onClickCapture={(event) => { const target = event.target as HTMLElement; if (readOnly && target.closest('.question-editor-body input,.question-editor-body select,.question-editor-body textarea,.question-editor-body button,.question-editor-body .file-picker') && !target.closest('.source-panel-collapse')) { event.preventDefault(); event.stopPropagation() } }} onKeyDownCapture={(event) => { if (readOnly && (event.target as HTMLElement).closest('.question-editor-body') && !['Tab', 'Escape'].includes(event.key)) event.preventDefault() }}>
    ...
    <div className={`question-editor-body${sourceCollapsed ? ' source-collapsed' : ''}`}>
      ...
      {question.type === 'programming' && question.programming && <ProgrammingEditor program={question.programming} readOnly={readOnly} setProgram={(programming) => setQuestion({ ...question, programming, reviewed: false })} updateCase={updateCase} />}</section>
      <section className="question-editor-section"><h3>答案解析</h3><MarkdownEditorField label="解析内容" rows={5} value={question.explanation_markdown ?? ''} readOnly={readOnly} onChange={(explanation_markdown) => setQuestion({ ...question, explanation_markdown, reviewed: false })} /></section>
...
function ProgrammingEditor({ program, readOnly, setProgram, updateCase }: { program: NonNullable<EditableQuestion['programming']>; readOnly: boolean; setProgram: (program: NonNullable<EditableQuestion['programming']>) => void; updateCase: (index: number, patch: Partial<ProgrammingCase>) => void }) {
  return <section className="program-editor"><div className="section-title"><div><h3><Code2 />编程规格</h3></div></div>
    ...
    <MarkdownEditorField label="输入格式" value={program.input_markdown} readOnly={readOnly} onChange={(input_markdown) => setProgram({ ...program, input_markdown })} /><MarkdownEditorField label="输出格式" value={program.output_markdown} readOnly={readOnly} onChange={(output_markdown) => setProgram({ ...program, output_markdown })} /><MarkdownEditorField label="数据范围" value={program.constraints_markdown} readOnly={readOnly} onChange={(constraints_markdown) => setProgram({ ...program, constraints_markdown })} /><label>初始代码<textarea className="code-input" rows={5} value={program.starter_code} onChange={(e) => setProgram({ ...program, starter_code: e.target.value })} /></label><label>参考程序<textarea className="code-input" rows={10} value={program.reference_solution ?? ''} onChange={(e) => setProgram({ ...program, reference_solution: e.target.value })} /></label>
    <h3>测试点</h3>{program.cases.map((item, index) => <div className="case-editor" key={item.id ?? index}><div><strong>{item.is_sample ? '公开样例' : '隐藏测试点'}</strong><label className="check-label"><input type="checkbox" checked={item.is_sample} disabled={readOnly} onChange={(e) => updateCase(index, { is_sample: e.target.checked, weight: e.target.checked ? 0 : item.weight })} />公开</label>{!item.is_sample && <label className="check-label"><input type="checkbox" checked={item.confirmed ?? false} disabled={readOnly} onChange={(e) => updateCase(index, { confirmed: e.target.checked })} />已确认</label>}</div><label>输入<textarea className="code-input" rows={3} value={item.input_data} readOnly={readOnly} onChange={(e) => updateCase(index, { input_data: e.target.value, confirmed: false })} /></label><label>期望输出<textarea className="code-input" rows={3} value={item.expected_output} readOnly={readOnly} onChange={(e) => updateCase(index, { expected_output: e.target.value, confirmed: false })} /></label>{item.is_sample && <MarkdownEditorField label="样例解释" value={item.explanation_markdown ?? ''} readOnly={readOnly} onChange={(explanation_markdown) => updateCase(index, { explanation_markdown, confirmed: false })} />}{!item.is_sample && <label>权重<input type="number" min="0" value={item.weight} readOnly={readOnly} onChange={(e) => updateCase(index, { weight: Number(e.target.value), confirmed: false })} /></label>}{!readOnly && <button type="button" className="danger-button" onClick={() => setProgram({ ...program, cases: program.cases.filter((_, itemIndex) => itemIndex !== index) })}><Trash2 />删除测试点</button>}</div>)}
    {!readOnly && <button type="button" className="ghost" onClick={() => setProgram({ ...program, cases: [...program.cases, { input_data: '', expected_output: '', is_sample: false, weight: 0, confirmed: false, note: '', explanation_markdown: '' }] })}><Plus />添加测试点</button>}
  </section>
}
