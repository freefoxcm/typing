...
import { ChevronDown, GripVertical, Pencil, Plus, Power, RefreshCcw, Trash2 } from 'lucide-react'
    ...
    <div className={`word-set-admin-list${reordering ? ' is-reordering' : ''}`} aria-busy={reordering}>{sets.map((item, index) => {
      ...
      return <SortableWordSetCard item={item} expanded={open} disabled={reordering || sets.length < 2} key={item.id}><header>
        ...
        <div className="library-card-actions"><button className="ghost" aria-label={`向单词集 ${item.title} 添加单词`} title="添加单词" onClick={(event) => openCreateWord(item, event.currentTarget)}><Plus />单词</button><AdminItemActionsMenu label={`单词集 ${item.title}`} actions={[
          ...
          { key: 'delete', label: '删除单词集', icon: <Trash2 />, danger: true, separatorBefore: true, onSelect: () => { if (window.confirm('删除单词集及全部词条？历史成绩会保留拼写快照。')) void action(() => api(`/api/admin/word-sets/${item.id}`, { method: 'DELETE' }), '单词集已删除') } },
        ]} /></div>
        ...
        {item.words?.map((word) => <div key={word.id}><code>{word.spelling}</code><span>{word.phonetic || '待补音标'}</span><p>{word.meaning_zh || '待补释义'}</p><i className={`word-status status-${word.enrichment_status}`}>{statusLabels[word.enrichment_status ?? ''] ?? word.enrichment_status}</i>{word.enrichment_error && <small title={word.enrichment_error}>查看错误</small>}<div className="compact-row-actions word-row-actions"><button className="ghost icon-button compact-icon-button" aria-label={`编辑单词 ${word.spelling}`} title="编辑单词" onClick={(event) => openEditWord(word, item, event.currentTarget)}><Pencil /></button><button className="ghost compact-text-button" onClick={() => void action(() => api(`/api/admin/words/${word.id}`, { method: 'PUT', ...jsonBody({ word_set_id: word.word_set_id, spelling: word.spelling, phonetic: word.phonetic, meaning_zh: word.meaning_zh, technical_meaning_zh: word.technical_meaning_zh, active: !word.active }) }), word.active ? '单词已停用' : '单词已启用')}>{word.active ? '停用' : '启用'}</button>{word.enrichment_status === 'failed' && <button className="ghost icon-button compact-icon-button" aria-label={`重试单词 ${word.spelling}`} title="重试单词" onClick={() => void action(() => api(`/api/admin/words/${word.id}/retry`, { method: 'POST' }), '单词已重新排队')}><RefreshCcw /></button>}<button className="danger-button icon-button compact-icon-button" aria-label={`删除单词 ${word.spelling}`} title="删除单词" onClick={() => window.confirm(`删除 ${word.spelling}？`) && void action(() => api(`/api/admin/words/${word.id}`, { method: 'DELETE' }), '单词已删除')}><Trash2 /></button></div></div>)}
