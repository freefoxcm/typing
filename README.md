...
题套可在“导入导出 → 习题题库”导出为 ZIP 迁移包，或从其他部署先预览再导入。迁移包包含当前题目、答案、复核状态、原题截图、题干配图、原始 PDF 和编程测试点，不包含学生成绩、错题、练习记录和识别任务。导入时可逐套跳过、创建副本或覆盖同源草稿；包体上限由 `QUESTION_BUNDLE_MAX_MB` 控制，默认 250 MB。草稿题套也可在题库标题栏上传或替换原始 PDF，替换后所有题目会恢复为待复核。

升级时拉取或替换代码，然后重新运行 `docker compose up -d --build`；容器启动会自动执行 Alembic 数据库迁移。

## 学习彩蛋

管理员在“彩蛋设置 → 学习彩蛋”开启功能，设置奖励时长、最低题量以及按分数解锁或随机游戏。默认关闭；默认至少 10 题、星光冒险 80 分、卡丁赛车 90 分、共享 15 分钟。成绩按实际得分与满分折算百分制，不取四舍五入后的显示值。仅整套习题与随机组题参与，错题重练、打字和单词练习不参与。

每名学生按北京时间每天最多一次奖励。开始前可通过更高成绩增加游戏选择，随机结果不重抽；未开始的奖励当天有效。结果页会播放可跳过的礼盒解锁特效，首页保留礼物入口，点击开始游戏后才连续计时。暂停、退出、刷新和切换游戏不延长截止时间。关闭总开关会撤销当前奖励，重新开启不恢复旧奖励，也不补发历史成绩。

两个游戏随应用镜像一起部署，不依赖原游戏目录。首次升级自动运行 `0014_easter_eggs` 数据库迁移。游戏进度按学生分别保存在浏览器，奖励资格和时间保存在 SQLite；赛车需要电脑键盘与 WebGL 2。浏览器验收方式见 [彩蛋验收说明](docs/easter-eggs.md)。
